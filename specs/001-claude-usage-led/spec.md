...
- Q: What thresholds separate the normal / warning / exhausted color bands of the five-hour utilization? → A: Defaults are `<70%` normal · `70%–<90%` warning · `≥90%` exhausted (half-open bands: warning includes its lower bound and excludes its upper). As of the same iteration that introduced configurable `ERROR_COLOR` / `STARTUP_COLOR`, the thresholds themselves are operator-configurable via `WARN_THRESHOLD_PERCENT` and `EXHAUSTED_THRESHOLD_PERCENT` (see FR-013); the three-band structure stays fixed.
...
3. **Given** the API server reports a five-hour utilization of 100% (window exhausted), **When** the ESP32 completes the poll cycle in which utilization first reaches 100%, **Then** the strip first holds the full-red proportional fill for that one poll as the "quota hit" confirmation, and on the next exhausted poll switches to the blue countdown-to-reset display (User Story 4 / FR-016).
...
Once the five-hour window is exhausted (utilization at 100% or above), the user still wants to know roughly how long until the window reopens. On the first poll where utilization reaches 100%, the strip first shows a full red bar for one poll cycle as a "quota hit" confirmation; on the next exhausted poll it switches to a distinct blue bar that fills toward reset: near-empty when a full five-hour window remains and growing to the full strip at the moment of reset — so by a glance at how much of the strip is lit blue the user can estimate "reset is soon" (nearly full) vs. "reset is still far away" (nearly empty) without opening a terminal.
...
**Independent Test**: Configure the API server to return five-hour utilization at 100% with a remaining-minutes-until-reset value of a known interval (e.g., 30). Power on the ESP32 and confirm the strip shows a blue bar lit to roughly `(300 − 30) / 300 ≈ 90%` of its length and then grows toward the full strip over the next 30 minutes, reaching fully lit at the end of the interval.

**Acceptance Scenarios**:

1. **Given** the API server reports five-hour utilization of 100% with a remaining-minutes-until-reset value of 60, **When** the ESP32 first observes utilization reach 100%, **Then** the strip holds the full-red proportional fill for one poll cycle (the "quota hit" confirmation), and on the next exhausted poll switches to a blue bar lit to roughly `(300 − 60) / 300 = 80%` of its length; over the following 60 minutes the lit portion grows roughly linearly toward the full strip, reaching fully lit at reset.
2. **Given** the strip is mid-countdown (utilization still ≥ 100%), **When** a new poll arrives reporting utilization back below 100% (e.g., the window has just reset), **Then** the strip exits the countdown and resumes the normal proportional-fill display from User Story 1 in the appropriate color band; if utilization later climbs back to 100%, the one-poll red confirmation plays again before the countdown resumes.
3. **Given** the strip is mid-countdown, **When** the locally tracked remaining minutes reach zero and no subsequent poll has yet returned fresh data, **Then** the strip is fully lit in blue and remains fully lit until the next successful poll updates the display.
4. **Given** the device reboots while utilization is still at 100%, **When** the first post-boot poll succeeds, **Then** the full-red bar plays once before the countdown resumes (the one-poll confirmation gate is RAM-only and rearms on every boot, by design).
...
- **Server returns utilization > 100%** (overage): the strip plays the one-poll full-red "quota hit" confirmation and then engages the blue countdown-to-reset visualization (User Story 4 / FR-016) exactly as it would for 100%; utilization magnitude above 100% does not change the display.
- **Remaining-minutes value is zero or negative at receipt** (the server's view says the window already rolled over but the upstream still reports ≥ 100% utilization): the countdown has zero remaining time, so the strip is fully lit in blue until the next poll updates the display.
...
- **FR-009**: The ESP32 MUST color the lit segment based on the current five-hour utilization, using a three-band scheme whose two boundaries are operator-configurable:
  ...
  - `WARN_THRESHOLD_PERCENT` ≤ utilization `<` `EXHAUSTED_THRESHOLD_PERCENT` (default `90`) → `WARN_QUOTA_COLOR` (yellow by default),
  - utilization `≥` `EXHAUSTED_THRESHOLD_PERCENT` → `EXHAUSTED_QUOTA_COLOR` (red by default).
  The bands are half-open: each includes its lower bound and excludes its upper, so a band change happens exactly at the threshold value (aligning the EXHAUSTED band with the red threshold marker — see FR-013). The thresholds MUST satisfy `0 < WARN_THRESHOLD_PERCENT ≤ EXHAUSTED_THRESHOLD_PERCENT ≤ 100`. The exhausted countdown (FR-016) still triggers strictly at utilization `≥ 100` regardless of `EXHAUSTED_THRESHOLD_PERCENT`.
...
- **FR-016**: When the current five-hour utilization is at or above 100%, the ESP32 MUST display a *countdown-to-reset* visualization in `COUNTDOWN_COLOR` (blue by default) in place of the proportional-usage fill:
  - on the first successful poll where utilization first reaches 100% (or on the first such poll after a reboot, since the gate is RAM-only), the firmware MUST hold the full-red proportional-usage fill for exactly one poll cycle as the "quota hit" confirmation, *then* switch to the blue countdown on the next exhausted poll; the gate MUST be rearmed whenever utilization drops below 100% so a later re-exhaustion replays the one-poll red confirmation,
  - the lit portion MUST grow approximately linearly toward the reset — zero LEDs when a full five-hour window remains, and the entire strip lit at the moment of reset,
  - the lit-LED count MUST be derived from the remaining-minutes-until-reset value of the most recent successful poll measured against the fixed five-hour (300-minute) window — `lit ≈ (300 − remaining_minutes) / 300 × NUM_OF_LED` — decremented locally between polls via the firmware's monotonic timer,
  - the strip MUST fill from the same fill end as the proportional-usage display (FR-008),
  - when the locally tracked remaining minutes reach zero the strip MUST be fully lit, and MUST remain fully lit until a subsequent successful poll updates the display,
...
- **LED display state**: The current visual mode of the strip — one of: *startup (white chasing animation)*, *usage display (proportional fill colored by quota band)*, *exhausted countdown (blue bar that fills from the fill end toward the full strip as the time-to-reset approaches)*, or *error (blinking red)*. At any moment exactly one state is active.
...
- **SC-007**: When the device is in the exhausted countdown, a user can correctly identify how close the reset is — to within a quarter of the five-hour window — on first glance, in at least 8 out of 10 trials.
...
- All five visual colors are user-configurable: the three band colors (`NORMAL_QUOTA_COLOR`, `WARN_QUOTA_COLOR`, `EXHAUSTED_QUOTA_COLOR`), the failure-blink color (`ERROR_COLOR`), and the startup-chase color (`STARTUP_COLOR`). The two utilization thresholds that select between the bands (`WARN_THRESHOLD_PERCENT`, `EXHAUSTED_THRESHOLD_PERCENT`) are also user-configurable, with defaults `70` and `90` respectively. The three-band structure is itself fixed (no fewer or greater number of color regions in v1).
